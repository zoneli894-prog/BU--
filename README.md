...
│   │   └── theme/              # Vue 组件和全局样式
│   ├── articles/               # 成员文章
│   ├── data/                   # 时间轴、组织架构、人物列表、文章清单、资料库、首页数据
│   ├── history/                # 历史沿革页面
│   ├── members/                # 成员资料（11 位成员）
│   ├── news/                   # 新闻动态
│   ├── public/
...
> ⚠️ 人物和文章是「清单 + 详情」两份数据：新增一位成员或一篇文章时，除了创建详情页，还要在对应的清单（`docs/data/member-grid.json` / `docs/data/articles.json`）里补一条记录，列表页才会显示。

### 直接编辑内容文件

- 文章、新闻和成员资料使用 Markdown；它们的**列表数据**在 `docs/data/*.json`。
- 时间轴、组织架构、首页轮播与动态、人物列表、文章清单、资料库清单使用 JSON。
- 图片放置在 `docs/public/images/`，提交前应进行压缩。
- 新闻列表页 `docs/news/index.md` 目前是手写卡片，链接请用相对路径（`./xxx`），写成 `/news/xxx` 在 GitHub Pages 子路径下会 404。
- 修改 JSON 后建议先运行 `npm run build`，避免格式错误导致部署失败。
- 内容口径（成员数、成立年份、核心层名单）以 [CLAUDE.md](CLAUDE.md) 的「内容口径」一节为准。
